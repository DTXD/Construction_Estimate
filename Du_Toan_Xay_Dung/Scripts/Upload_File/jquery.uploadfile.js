...
(function ($) {
    if ($.fn.ajaxForm == undefined) {
        $.getScript("/Scripts/libs/jquery.form.js");
    }
    var feature = {};
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            extErrorStr: "không được thêm vào. Phải thêm đúng định dạng ảnh: ",
            duplicateErrorStr: "không được thêm vào. Hình đã được chọn.",
            sizeErrorStr: "không được thêm vào. Kích thước ảnh: ",
            uploadErrorStr: "Upload is not allowed",
            maxFileCountErrorStr: " không được thêm vào. Số hình ảnh tối đa :",
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for (var i = 0; i < files.length; i++) {
                if (s.carImage) {
                    if ($("#ajax-upload-list .ajax-file-upload-statusbar > img.ajax-file-upload-preview").length == 14)
                        continue;
                ...
                if (s.carImage) {
                    pd.statusbar.attr("rel", files[i].size);
                    
                    totalUploadSize += files[i].size;
                    var convertToMB = totalUploadSize > 0 ? (totalUploadSize / 1024 / 1024) : 0;
                    $("#divTotalUploadSize").html(convertToMB.toFixed(2) + " MB");
                }
                //var fileNameStr = "";
                //if (s.showFileCounter)
        ...
        function createProgressDiv(obj, s) {
            ...
            if (s.showQueueDiv)
                ...
                if ($("#ajax-upload-list").length > 0) {
                    $("#ajax-upload-list").append(this.statusbar);
                    $("#ajax-upload-list").sortable();
                    $("#ajax-upload-list").disableSelection();
                } else {
        ...
        function ajaxFormSubmit(form, s, pd, fileArray, obj, file) {
            ...
            var options = {
                ...
                success: function (data, message, xhr) {
                    ...
                    if (s.carImage && data != null && data.data.length > 0) {
                        pd.statusbar.attr("id", "pItem-" + data.data[0].CarImageId);
                        pd.preview.attr("src", data.data[0].NormalUrl);
                        pd.cancel.click(function () {
                            var id = $(this).parent().attr("id");
                            if (id != null && id != '') {
                                deleteImage(id.substr(6));
                            }
                        });
                    }
                    pd.abort.hide();
